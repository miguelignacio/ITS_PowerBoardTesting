...
def VoltageScan(output, master, channel, step):
	...
	if(Iread>0): rload = Vread/Iread
        ...
        line = "%d %5d %10.2f %8.2f %8.2f %8.d %4.f %4.d" % (channel, voltage, Vread, Iread, rload, start, end, step)
    ...
    print 'Latch status is ' , PrintLatchStatus(master)
    print 'end of voltage scan for channel %d' %(channel)
    print '################################################## \n'	
    return
